...
- **On the filigree, placement is not the lever, and the tool must say so.**
  The curve is a flat band that never approaches zero, and the un-optimised 168
...
matplotlib to rasterise and it silently filled the 7 holes: material read 52.7%
...
numpy scanline rasteriser agreed with matplotlib **100.000%** on the filigree,
...
the table; they are statements about the rasteriser, not about the shipped
configuration.)
...
gained ~5%; the cost is labelling and erosion, and there is no clever way
around it.

## Approach (approved 2026-09-06): rasterise the material region, label components

Rasterise one tile of the pattern once, fill-rule aware. Per gore, build a
...
choice.** The cheaper variant — rasterise in master space, where the tiling is
...
- **Exact polygon booleans** (Vatti, Greiner–Hormann) give exact areas and no
  ...
  and nothing else. The precision buys nothing: the floors are in millimetres
...
1. **Any filled element is material; the colour is not interpreted.** One
   colour is sufficient — it marks each contour's interior as one polarity and
   the background as the other — and letting colour select polarity would
   invent a convention the user then has to remember. If a file carries more
   than one distinct fill colour, the operator reports it as a note ("3 fill
   colours found, all treated as material") so an unwanted background rectangle
   is visible rather than silently swallowing the tile. No new control.
2. **Fill-rule is honoured, both rules implemented.** Neither sample file
...
def _iter_gore_frames(pattern, placements, outlines, circumference,
    ...
    """_gore_geometry + _tile_origins, behaviour identical to today."""
```
...
### Sampling detail: the tile mask is rasterised at half the gore pitch
...
unbounded at a bare apex. At equal pitch a nearest-neighbour lookup would begin
skipping master pixels near the top. Rasterising the tile at `px / 2` keeps the
...
`tile_lookup(inverse_warp(pixel)) ∧ inside_gore ∧ y ≤ pattern_top`, labelled
...
and it costs nothing, because it falls out of the labelling. A small shape that
...
**3×3 square, not the 4-neighbour plus**: a plus-shaped ball is *smaller* than
the disc of the same radius, so 4-neighbour erosion lets thin shapes survive —
...
strips by at most √2. It also matches the 8-connectivity used for labelling.
...
intended behaviour: the width floor is a guard against the hair-thin-but-long
...
Rasterising is an estimate, and the spec should not pretend otherwise.
...
- **"Best placement is no better than the current one"** — the filigree case.
  ...
  stopped a cut file shipping in the belief it was optimised.
...
stroked in a distinct colour, unfilled. Rectangles rather than traced component
...
- **Gore-phase periodicity** — the dangerous optimisation. With all outlines
...
- Sub-pixel fragments are not counted; `px` derivation and its clamps; the
  ...
  `PatternError`; multiple fill colours produces the note.
