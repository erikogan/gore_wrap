...
connected-component labelling, erosion) supports a rewritten `pattern_fit.py`
that rasterises one pattern tile, renders each gore in final SVG millimetres by
...
- **All lengths are millimetres**, matching `svg_export` and `pattern_warp`.
- **The export path must stay behaviourally inert** through Tasks 6 and 7. The
...
- Produces: `raster.fill(rings, nx, ny, px, even_odd=False) -> np.ndarray`
  ...
  each a closed ring in millimetres with **y up**; the raster's origin is the
  lower-left corner of pixel `(0, 0)`, and pixel `(r, c)` has its centre at
  `((c + 0.5) * px, (r + 0.5) * px)`.
...
git commit -m "Add a numpy scanline polygon rasteriser"
```
...
def erode(mask, steps=1):
    ...
    Square rather than the 4-neighbour plus, and the choice decides which way
...
class Pattern:
    ...
    def fill_colors(self):
        """Sorted distinct fills, for the operator's multi-colour note."""
...
def _element_fill(element):
    ...
    and the behaviour we want -- such a shape is material.
    """
...
- Produces:
  ...
  - `_iter_gore_frames` keeps its exact current signature and behaviour.
...
- Produces:
  ...
  - `pattern_fit.build_tile(pattern, circumference, repeats_x, px) -> TileMask`
    — rasterises at `px / 2` (the returned `TileMask.px` is that half pitch).
...
git commit -m "Rasterise the pattern tile with fill, nesting and welding"
```
...
changes — the `search_placement` call, `_on_success`, and a multi-colour note:
...
In `GOREWRAP_OT_optimize_placement.execute`, beside the multi-colour note added
in Step 3:
...
```python
def test_defect_boxes_are_returned_in_final_millimetres(tmp_path):
...
Initialise `defect_rects = None` beside `pattern_polys = None` at the top, add
