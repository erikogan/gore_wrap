...
trade-off table, apply it, and run Optimize again. Strips are also set by count
now rather than by angle, since a whole number of strips is what you cut. This
release raises the minimum Blender version to 4.5.
...
- **Strips are set by count now, not by angle.** The **Strips** panel asks for
  a strip count from 8 to 72, and shows the angle each strip works out to
  underneath. It used to be the other way around: you set an angle and the
  count it snapped to was the readout. A whole number of strips is what you
  actually cut, so it is what you should be able to type. The range is
